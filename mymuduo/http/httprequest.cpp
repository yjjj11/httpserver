...
bool httprequest::parse_header(string& line)
{
    ...
    if(line.empty())
    {
        ...
        {
            cout<<"进入了： "<<header_.count("Content-Length")<<"\n";
            cout<<header_.find("Content-Length")->second<<"\n";
...
void httprequest::parse_POST()
{
    ...
    if(method_ == "POST" && header_["Content-Type"] == "application/x-www-form-urlencoded") {
        ...
        if(choice_in.count(path_)) {//分辨是登陆还是注册
            ...
            if(tag == 0 || tag == 1) {
                bool isLogin = (tag == 1);
                if(isLogin)cout<<"处理登陆业务\n";
                else cout<<"处理注册业务\n";
                if(UserVerify(post_["username"], post_["password"], isLogin)) {
                    path_ = "/welcome.html";
                    cout<<"UserVerify Sucessiful\n";
                } 
                else {
                    path_ = "/error.html";
                    cout<<"UserVerify failed\n";
                }//根据验证是否成功来跳转
            }
        }
    }   
