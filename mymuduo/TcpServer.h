...
unique_ptr<EventLoop> mainloop_;        //多线程情况下一个TcpServer类可以有多个事件循环
    ...
    std::function<void(spConnection,  vector<string>&)> onmessagecb_;  // 回调EchoServer::HandleMessage()。
...
public:
    ...
    void onmessage(spConnection conn, vector<string>& message);
    ...
    void setonmessagecb(std::function<void(spConnection,  vector<string>&)> fn);
